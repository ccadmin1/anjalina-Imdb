...
async def filter(client, message):
    ...
    if AUTH_CHANNEL:
        ...
        try:
            ...
            await client.send_message(
                chat_id=message.from_user.id,
                text="**Hey..Bruh🙋‍♂️...Please Join My Channel to use Me!**",
    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            ...
            await client.send_sticker(chat_id=message.from_user.id, sticker='CAACAgUAAxkBAALRrWFoDOAHt5HCuZIbsGBgy7-xEY8ZAAKdDgACiQABcx-J5shgP411CSEE')
            return
    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
            ...
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                await query.answer("Hey..Bruh🙋‍♂️...Please Join My Updates Channel to use Me👹!",show_alert=True)
                return
            ident, file_id = query.data.split("#")
            filedetails = await get_file_details(file_id)
            for files in filedetails:
